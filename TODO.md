...
- Latest deploy: GitHub Actions run `26824049548` succeeded at `2026-06-02T13:49:34Z`; Fly release v22, image `deployment-01KT49FP0P0EN7JZMTP9FEBD0Z`, code commit `cb173f0`. Current RUA batch is local-only until the next push/deploy.
- Corpus: `802` verified entries, `62` symptom guides, `209` official sources, `865` generated indexable pages.
- Local verification passed so far for the Dometic RUA exact-fault support-alias batch: RED/GREEN focused corpus regressions for RUA exact owner-search wording, 22 official support URLs, owner-visible AC-supply/battery/door/gas-bottle/gas-valve boundaries, and support-only source mapping; `npm run validate:corpus`, focused `npm run test:unit -- src/lib/corpus.test.ts`, full `npm run test:unit`, `npm run source:audit`, `npm run traffic:report`, `npm run verify:runtime`, `git diff --check`, bounded JSON diff-shape audit, all 22 direct support URL checks, and built-app desktop/mobile route/search smoke for 7 RUA routes plus 3 searches. Build splits corpus to `dist/assets/corpus-DkYBZGzA.json` at `1311.43 kB` / `88.83 kB` gzip and main JS to `210.16 kB` / `65.83 kB` gzip with no Vite chunk warning.
- Reviews passed: source/safety explorer verified all 22 RUA URLs are official `support.dometic.com` pages, reset text is present, `05` mentions low AC supply, `09`/`22` mention battery checks, `10` mentions door closure, `50`-`53` mention gas bottle/gas valves, and no page supports board/fuse/wiring/burner/regulator/sensor/control DIY work. Data-quality review approved IDs/slugs/refs/counts/routes/searches and confirmed no existing entries, sources, or symptoms changed.
- Live verification pending for the RUA batch after deploy. Last live verification remains RUC: `/corpus-stats.json` reports `780`/`62`/`187`/`843`; `/sitemap.xml` and 7 representative new Dometic RUC routes returned HTTP `200`; production desktop/mobile Playwright smoke checked 7 RUC routes plus 3 searches with `0` console/page/request failures and confirmed unsafe fuse/wiring owner wording is absent.
...
- Next automated batch goal: Add remaining official Dometic RM10 refrigerator support-page fault aliases from sitemap-verified Fault pages while preserving manual-backed RM10/RMD/RML rows.
