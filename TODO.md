...
- Latest deploy: GitHub Actions run `26831849072` succeeded at `2026-06-02T15:59:49Z`; Fly release v27, image `deployment-01KT4GYQKTVY3CJGVAYH6SDCV7`, code commit `5e2df8b`.
- Corpus: `819` verified entries, `85` symptom guides, `271` official sources, `905` generated indexable pages.
- Local verification passed for the Dometic RUA/RUC symptom support batch: RED focused unit lookup/source tests and RED focused Playwright lookup test failed for missing data/routes, focused GREEN unit tests, focused GREEN Playwright, `npm run validate:corpus`, full `npm run test:unit` (`71` tests), `npm run source:audit`, `npm run traffic:report`, `git diff --check`, and `npm run verify:runtime` (`12` browser tests, `905` generated pages). Read-only source/safety and data/UI reviewers reported no findings; one invalid static-HTML route smoke was discarded because symptom content hydrates through JS.
...
- Live verification passed: `/corpus-stats.json` reports `819`/`85`/`271`/`905`; `/sitemap.xml` reports `905` URLs; production desktop/mobile Playwright smoke checked all 13 new RUA/RUC symptom routes, 10 representative owner-search lookups (`rua does not work automatic mode`, `rua not cooling lower temperature`, `rua 12v battery management`, `rua defrost ice`, `rua eco mode defrost`, `ruc no display`, `ruc compressor high ambient`, `ruc water leaking floor`, `ruc unusual noise fan`, `ruc too cold warmest setting`), and `0` console/page/request failures.
...
- Next automated batch goal: Triage official Norcold/Thetford refrigerator symptom-only support and owner-manual pages for cooling, power, door, ventilation, water, low-temperature, and service-only boundaries; add only non-duplicate owner-safe guides.
