...
- Latest deploy: GitHub Actions run `26835776957` succeeded at `2026-06-02T17:11:57Z`; Fly release v29, image `deployment-01KT4N3ATAFA3XTRRVK00YS4MR`, code commit `5647df4`.
- Latest deployed corpus: `819` verified entries, `105` symptom guides, `301` official sources, `925` generated indexable pages.
- Pending local Suburban/Airxcel batch corpus before deployment: `819` verified entries, `114` symptom guides, `302` official sources, `934` generated indexable pages.
...
- Live verification passed: live corpus asset reports `819`/`105`/`301`/`925`; `/sitemap.xml` returned HTTP 200; representative Furrion symptom and exact-code routes returned HTTP 200; production desktop/mobile Playwright smoke checked Furrion symptom lookup, service-boundary route content, exact `furrion 10.6 e3 compressor` code ranking, and found `0` console/page errors.
...
- Source triage note: accepted official Suburban FAQ, Suburban furnace guide, Suburban tank water-heater manual, Suburban tankless water-heater manual, and Suburban ST42/ST60 manual for owner-safe furnace and water-heater symptom guides. Rejected/deferred SF-VH ducting guidance, ST42/ST60 sell sheet, and support-video pages to the next model-specific batch; rejected forums, mirrors, and service-only gas, burner, wiring, fuse, control-board, aftermarket-element, and electrical repair procedures as owner guidance.
- Next automated batch goal: Triage official Suburban/Airxcel model-specific furnace ducting and ST42/ST60 cold-inlet or winter-use support pages for register/return-air sizing, thermostat placement, rapid cycling, minimum flow, temperature underperformance, and service-only boundaries; add only non-duplicate owner-safe guides.
