...
- Latest deploy: GitHub Actions run `26830129061` succeeded at `2026-06-02T15:30:19Z`; Fly release v26, image `deployment-01KT4F8MF3BT6EJ9XESG94S909`, code commit `c5f357f`. New RUA/RUC symptom batch is local pending push/deploy verification.
- Corpus: `819` verified entries, `85` symptom guides, `271` official sources, `905` generated indexable pages.
- Local verification passed for the Dometic 10-series RMD10/RML10/RMS10 owner-search alias batch: RED/GREEN unit alias lookup regression, RED/GREEN Playwright alias lookup regression, RED/GREEN `searchAliases` validator guard, duplicate RM10 support URL guard, rejected RMD/RML/RMS support roots and generic/non-symptom URL guards, no new source rows/code entries/routes, read-only source/safety + data/UI reviews with no findings, `npm run validate:corpus`, focused `npm run test:unit -- src/lib/corpus.test.ts -t "rejects malformed symptom search aliases"`, full `npm run test:unit` (`69` tests), `npm run source:audit`, `npm run traffic:report`, `npm run verify:runtime` (`10` browser tests), and `git diff --check`.
...
- Live verification passed: `/corpus-stats.json` reports `819`/`72`/`242`/`892`; `/sitemap.xml` reports `892` URLs; production desktop/mobile Playwright smoke checked `rmd10 gas smell`, `rml10 ammonia smell`, `rms10 defrost ice`, `rmd10 internal batteries`, `rml10 door ice compartment`, and `rms10 not cooling level` with `12` expected RM10 symptom-result links and `0` console/page/request failures.
