...
- Current local corpus pending deploy: `819` verified entries, `72` symptom guides, `242` official sources, `892` generated indexable pages.
- Local verification passed for the Dometic RM10 symptom/support-page batch: RED/GREEN focused corpus regressions for 16 sitemap-verified official `support.dometic.com` symptom pages, 10 symptom-only guides with no new code entries, owner-safe gas/ammonia/defrost/cleaning/door/winter-cover/internal-battery boundaries, exclusion of generic/duplicate/non-symptom pages, symptom lookup search coverage, manual-backed RM10/RMD/RML row preservation, `npm run validate:corpus`, focused `npm run test:unit -- src/lib/corpus.test.ts`, full `npm run test:unit`, `npm run source:audit`, `npm run traffic:report`, `npm run verify:runtime`, `git diff --check`, built-app desktop/mobile route/search smoke for 10 RM10 symptom routes plus 4 searches, and read-only source/safety + data/test reviews pass.
