...
- Local verification passed for the Dometic RUC exact-fault support-alias batch: RED/GREEN focused corpus regressions for RUC exact owner-search wording, 15 official support URLs, owner-visible battery/door/warm-goods/ventilation boundaries, and support-only source mapping; focused `npm run test:unit -- src/lib/corpus.test.ts` and `npm run validate:corpus` have passed so far. Full local verification is still pending before commit.
