...
- Latest deploy: GitHub Actions run `26826744002` succeeded at `2026-06-02T14:34:04Z`; Fly release v24, image `deployment-01KT4C2B9PAWRC6SD7EH3216XT`, code commit `dc27aa2`.
- Corpus: `819` verified entries, `62` symptom guides, `226` official sources, `882` generated indexable pages.
- Local verification passed for the Dometic RM10 `Fault:` support-page alias batch: RED/GREEN focused corpus regressions for 17 sitemap-verified official `support.dometic.com` fault pages, exact support-search wording, one-source mapping, owner-visible mode/source/door/winter/controller checks, service-only LP/electrical/display/wiring/control boundaries, exclusion of generic selector pages, and manual-backed RM10/RMD/RML row preservation; `npm run validate:corpus`, focused `npm run test:unit -- src/lib/corpus.test.ts`, full `npm run test:unit`, `npm run source:audit`, `npm run traffic:report`, `npm run verify:runtime`, `git diff --check`, built-app desktop/mobile route/search smoke for 7 RM10 routes plus 4 searches, and read-only source/safety + data/test reviews pass.
- Source/gap explorers verified the 17 new RM10 `Fault:` page candidates and confirmed existing numeric display-fault/petrol-pump aliases plus manual-backed RM10/RMD/RML rows should remain separate.
- Live verification passed: `/corpus-stats.json` reports `819`/`62`/`226`/`882`; `/sitemap.xml` and 7 representative new Dometic RM10 `Fault:` routes returned HTTP `200`; production desktop/mobile Playwright smoke checked 7 RM10 routes plus 4 searches with `0` console/page/request failures and confirmed unsafe service terms are absent from owner-safe sections.
...
- Next automated batch goal: Triage remaining official Dometic RM10 refrigerator symptom/support pages from sitemap-verified support URLs, starting with gas-operation despite mains supply, cooling, smell, door, defrost, and internal-battery pages; add only owner-safe symptom aliases.
