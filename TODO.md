...
- Latest deploy: GitHub Actions run `26824049548` succeeded at `2026-06-02T13:49:34Z`; Fly release v22, image `deployment-01KT49FP0P0EN7JZMTP9FEBD0Z`, code commit `cb173f0`.
- Corpus: `780` verified entries, `62` symptom guides, `187` official sources, `843` generated indexable pages.
- Local verification passed for the Dometic RUC exact-fault support-alias batch: RED/GREEN focused corpus regressions for RUC exact owner-search wording, 15 official support URLs, owner-visible battery/door/warm-goods/ventilation boundaries, and support-only source mapping; `npm run validate:corpus`, focused `npm run test:unit -- src/lib/corpus.test.ts`, full `npm run test:unit`, `npm run source:audit`, `npm run traffic:report`, `npm run verify:runtime`, `git diff --check`, bounded JSON diff-shape audit, and built-app desktop/mobile route/search smoke for 7 RUC routes plus 3 searches. Build splits corpus to `dist/assets/corpus-CSlBk0xV.json` at `1267.64 kB` / `86.77 kB` gzip and main JS to `210.16 kB` / `65.83 kB` gzip with no Vite chunk warning.
- Reviews passed: source/safety review verified all 15 RUC URLs are official `support.dometic.com` pages, owner actions stay visible-only, and no fuse/wiring/control-board work leaked into owner guidance. Data-quality review approved IDs/slugs/refs/counts/routes/searches and confirmed no existing entries, sources, or symptoms changed.
- Live verification passed: `/corpus-stats.json` reports `780`/`62`/`187`/`843`; `/sitemap.xml` and 7 representative new Dometic RUC routes returned HTTP `200`; production desktop/mobile Playwright smoke checked 7 RUC routes plus 3 searches with `0` console/page/request failures and confirmed unsafe fuse/wiring owner wording is absent.
